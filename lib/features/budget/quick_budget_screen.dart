  ...
  Future<void> _save() async {
    ...
    for (final cat in _categories) {
      ...
      final amount = double.tryParse(
        (_controllers[cat.id]?.text ?? '').replaceAll('.', '').replaceAll(',', ''),
      );
      if (amount == null || amount <= 0) continue;

      await repo.insert(BudgetModel(
        id: repo.newId(),
        categoryId: cat.id,
        limitAmount: amount,
        year: now.year,
    ...
    if (count == 0) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Pilih minimal 1 kategori dan isi nominal')),
        );
      }
      return;
    }
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            ...
            child: Text(
              'PILIH SEMUA',
              style: GoogleFonts.spaceGrotesk(fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 0.5, color: NeoBrutalColors.primary),
            ),
          ),
        ],
      ),
      body: Column(
        children: [
                  ...
                  child: NeoCard(
                    ...
                    child: Row(
                      children: [
                        // Checkbox
                        GestureDetector(
                          onTap: () => setState(() => _selected[cat.id] = !isChecked),
                          child: Container(
                            width: 28,
                            height: 28,
                            decoration: BoxDecoration(
                              color: isChecked ? NeoBrutalColors.success : Colors.transparent,
                              border: Border.all(
                                color: isChecked ? NeoBrutalColors.success : NeoBrutalColors.ink,
                                width: 2,
                              ),
                            ),
                            child: isChecked
                        ...
                        Container(
                          ...
                          child: Icon(Icons.category_rounded, size: 16, color: cat.colorValue),
                        ),
                            ...
                            children: [
                              Text(
                                ...
                                style: GoogleFonts.spaceGrotesk(
                                  ...
                                  letterSpacing: 0.5,
                                  color: isChecked ? NeoBrutalColors.ink : NeoBrutalColors.muted,
                                ),
                              ),
                              if (isChecked)
                                Padding(
                                  ...
                                  child: TextField(
                                    ...
                                    decoration: InputDecoration(
                                      hintText: 'Rp',
                                      hintStyle: GoogleFonts.spaceGrotesk(fontSize: 14, fontWeight: FontWeight.w500, color: NeoBrutalColors.muted),
                                      isDense: true,
                                      contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8),
                                      border: OutlineInputBorder(
                                        borderRadius: BorderRadius.zero,
                                        borderSide: const BorderSide(color: NeoBrutalColors.ink, width: 2),
                                      ),
                                      ...
                                      focusedBorder: OutlineInputBorder(
                                        borderRadius: BorderRadius.zero,
                                        borderSide: const BorderSide(color: NeoBrutalColors.primary, width: 2),
                                      ),
                                    ),
                                  ),
                                ),
          ...
          Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              border: Border(top: BorderSide(color: NeoBrutalColors.ink, width: AppConstants.borderPrimary)),
            ),
            child: SizedBox(
              width: double.infinity,
              child: NeoButton(
                ...
                color: selectedCount > 0 ? NeoBrutalColors.success : NeoBrutalColors.muted,
